...
increaseSim <- function(outList, nSim=10000)
{
  control = outList$control
  k = ncol(control$data)
  nMC = nrow(outList$MC)
  tail_ini = outList$MC[nMC,1]
  gamma_ini = outList$MC[nMC,2 + 4*(0:(k-1))]
  omega_ini = outList$MC[nMC,3 + 4*(0:(k-1))]
  alpha_ini = outList$MC[nMC,4 + 4*(0:(k-1))]
  beta_ini =  outList$MC[nMC,5 + 4*(0:(k-1))]
  if(k > 1){
    a_ini = outList$MC[nMC, 2 + 4*k]
    b_ini = outList$MC[nMC, 3 + 4*k]
  }else{
    a_ini = 0.03
	b_ini = 0.8
  }
  ...
  outList$H = outNewSim$H #(nMC*outList$H + nSim*outNewSim$H)/(nMC + nSim)
  outList$IC = outNewSim$IC
  outList$MC = as.mcmc( rbind(outList$MC, outNewSim$MC) )
  outList$elapsedTime = outList$elapsedTime + outNewSim$elapsedTime
